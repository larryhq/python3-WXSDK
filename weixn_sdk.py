...
class WxPayConf_pub(object):
    ...
    JS_API_CALL_URL = "http://qodome.com.cn/insurances/reciveEvent/paytest/"

    # =======【证书路径设置】=====================================
    # 证书路径,注意应该填写绝对路径
    SSLCERT_PATH = "{}/cacert/apiclient_cert.pem".format(os.path.abspath(sys.argv[0]))
    SSLKEY_PATH = "{}/cacert/apiclient_key.pem".format(os.path.abspath(sys.argv[0]))
...
class CurlClient(object):
    ...
    def postXmlSSL(self, xml, url, second=30, cert=True, post=True):
        ...
        self.curl.setopt(pycurl.HEADER, ["Content-Type: application/xml"])
        #设置证书
...
class JsApi_pub(Common_util_pub):
    ...
    def getOpenid(self):
        ...
        self.openid = json.loads(data.decode('gb2312'))["openid"]
        return self.openid
